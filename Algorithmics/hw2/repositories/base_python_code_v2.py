...
def generate_operation_sequence(q, s, max_time=10000):
    ...
    for queue_num in range(1, q + 1):
        dataStructures.append(f"Q{queue_num}")
    for stack_num in range(1, s + 1):
        dataStructures.append(f"S{stack_num}")
    ...
    for time in range(1, max_time + 1):
        ...
        for dataStrcutureNum in range(1, len(dataStructures) + 1):
            insertion_prob = calculate_insertion_prob(dataStrcutureNum, q, time, max_time)
            deletion_prob = calculate_deletion_prob(dataStrcutureNum, q, time, max_time)

            if random.random() < insertion_prob:
                dsIndex =  dsIndex + 1
                if dsIndex > len(dataStructures):
                    dsIndex = 0
                operations.append(f"{timestamp} INSERT {dataStructures[dsIndex]}, {unique_id}")
                unique_id += 1
                ...
                dsIndex =  dsIndex - 1
                if dsIndex < 0:
                    dsIndex = len(dataStructures)
                operations.append(f"{timestamp} REMOVE {dataStructures[dsIndex]}")
